...
bool Model::LoadTexture(const std::string& directoryPath, const std::string& filename)
{
	...
	// テクスチャバッファにデータ転送
	result = texbuff[number]->WriteToSubresource
	(
		0,
		nullptr, // 全領域へコピー
		img->pixels,    // 元データアドレス
	...
	D3D12_RESOURCE_DESC resDesc = texbuff[number]->GetDesc();
	...
	(
		texbuff[number].Get(), //ビューと関連付けるバッファ
		&srvDesc, //テクスチャ設定情報
		cpuDescHandleSRV
	);
...
void Model::Update(XMMATRIX matWorld, XMMATRIX matView, XMMATRIX matProjection)
{
	HRESULT result;
